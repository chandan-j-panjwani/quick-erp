...
class Customer
{
    ...
    {
        $query = "SELECT customers.id, customers.first_name, customers.last_name, customers.gst_no, customers.phone_no, customers.email_id, customers.gender, address.id AS address_id, address.block_no, address.street, address.city, address.pincode, address.state, address.country, address.town FROM customers INNER JOIN address_customer ON customers.id = address_customer.customer_id INNER JOIN address ON address_customer.address_id = address.id WHERE customers.deleted = 0";
        $totalRowCountQuery = "SELECT COUNT(*) AS total_count FROM customers INNER JOIN address_customer ON customers.id = address_customer.customer_id INNER JOIN address ON address_customer.address_id = address.id WHERE customers.deleted = 0";
        $filteredRowCountQuery = "SELECT COUNT(*) AS total_count FROM customers INNER JOIN address_customer ON customers.id = address_customer.customer_id INNER JOIN address ON address_customer.address_id = address.id WHERE customers.deleted = 0";
        ...
        for ($i = 0; $i < $numRows; $i++) {
            ...
            $subArray[] = <<<BUTTONS
            <a href="{$basePages}edit-customer.php?id={$fetchedData[$i]->id}" class='btn btn-outline-primary btn-sm'><i class="fas fa-pencil-alt"></i></a>
...
BUTTONS;
    ...
    {
        $query = "SELECT customers.id, customers.first_name, customers.last_name, customers.gst_no, customers.phone_no, customers.email_id, customers.gender, address.id AS address_id, address.block_no, address.street, address.city, address.pincode, address.state, address.country, address.town FROM customers INNER JOIN address_customer ON customers.id = address_customer.customer_id INNER JOIN address ON address_customer.address_id = address.id WHERE customers.id = {$id} AND customers.deleted = 0";
    ...
    public function delete($id)
    {
        try {
            $this->database->beginTransaction();
            $result = $this->database->raw("SELECT * FROM address_customer WHERE customer_id = $id")[0];
            $address_id = $result->address_id;
            $address_customer_id = $result->id;
            $this->database->delete($this->table, "id = {$id}");
            $this->database->delete('address', "id = {$address_id}");
            $this->database->query("DELETE FROM address_customer WHERE id = {$address_customer_id}");
            $this->database->commit();
