...
<script src="<?= BASEASSETS; ?>js/pages/supplier/manage-suppliers.js"></script>
<script>
    toastr.options = {
        "closeButton": true,
        "debug": false,
    ...
    ?>
        toastr.error("Failed to update a supplier", "Failed");
    <?php
        Session::unsetSession(UPDATE_ERROR);
    elseif (Session::hasSession(UPDATE_SUCCESS)) :
    ?>
        toastr.success("Supplier has been updated successfuly !", "Updated!!")
    <?php
        Session::unsetSession(UPDATE_SUCCESS);
    elseif (Session::hasSession(DELETE_SUCCESS)) :
    ?>
        toastr.success("Supplier has been deleted successfuly !", "Success")
    <?php
        Session::unsetSession(DELETE_SUCCESS);
    elseif (Session::hasSession(DELETE_ERROR)) :
    ?>
        toastr.error("There was some problem deleting the supplier!", "Failed!")
    <?php
